$(document).ready(function() {
  ...
  console.log(
    "to win: attack right to left, attacking left to right will result in loss"
  );
  ...
  function reset() {
    for (i = 0; i < 4; i++) {
      ...
      $(document.getElementById("character" + (i + 1))).addClass(
        "col-md-2 border border-dark px-4 my-3 mx-auto chose-character bg-black"
      );
      //reset chrarecter stats
      $(document.getElementById("character" + (i + 1))).attr(characterArray[i]);
      $(document.getElementById("character" + (i + 1) + "hp")).text(
        characterArray[i].hp
      );
  ...
  $(document).on("click", "#attack-button", function() {
    ...
    var heroDisplayHp = document.getElementById(
      "character" + currentHeroId + "hp"
    );
    var enemyDisplayHp = document.getElementById(
      "character" + currentEnemyId + "hp"
    );
    ...
    if (heroHp >= 0 && enemyHp >= 0) {
      ...
      $("#attack-values").text(
        "You attacked for " +
          heroAttack +
          " HP damage! You got counter-attacked for " +
          enemyCounter +
          " HP damage :O"
      );
    }
